...
function OrdersPage({ orders, currentUser }) {
  ...
  const filtered = orders.filter(o => {
    const d = (o.created_at || o.order_id || "").slice(0,10);
    if (dateFilter === "today"  && d !== today) return false;
    if (dateFilter === "month"  && d.slice(0,7) !== today.slice(0,7)) return false;
...
function FinancePage({ orders, expenses, setExpenses }) {
  ...
  const monthOrders   = orders.filter(o => (o.created_at||"").slice(0,7) === today.slice(0,7));
  const monthExpenses = expenses.filter(e => (e.created_at||"").slice(0,7) === today.slice(0,7));
